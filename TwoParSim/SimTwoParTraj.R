...
for (chckpt in 0:9) {
  bfd <-
    foreach(
      i = seq(1 + chckpt * 10, min(10 + chckpt * 10, 98), length.out = 10),
      .combine = "rbind"
    ) %:%
    foreach(
      j = seq(
        0.8 / simTrajectory$R0[i],
        1.2 / simTrajectory$R0[i],
        length.out = nS0
      ),
      .combine = "rbind"
    ) %:%
    foreach(
      k = seq(0.0002 / 10, 0.0002 * 10, length.out = nI0),
      .combine = "rbind"
    ) %dopar%
    {
      if (is.na(simTrajectory$amp[i])) {
        return(data.frame(
          t = simTrajectory$t[i],
          S = NA,
          I = NA,
      ...
      bfddataper <- bfddataper %>%
        rename(t = time) %>%
        mutate(t = simTrajectory$t[i])
      return(bfddataper)
    }
  saveRDS(
    bfd,
    file = paste("bfd/sim_trajectory_", chckpt + 1, ".rds", sep = "")
  )
  print(paste(
    "sim_trajectory_",
    chckpt + 1,
    ".rds created succesfully",
    sep = ""
  ))
}
